...
export default function SignificancePrediction() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const features = [
      ...
      // Add more features as needed for the significance model
    ]

    try {
      const response = await fetch('/api/predict-significance', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Earthquake Significance Prediction</h1>
      ...
      {prediction !== null && (
        <div className="mt-6 p-4 bg-gray-100 rounded">
          ...
          <p>Significance: {prediction.toFixed(2)}</p>
        </div>
      )}
    </div>
  )
